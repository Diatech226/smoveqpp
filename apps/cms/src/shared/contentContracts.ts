...
export const normalizeSlug = (value: string | undefined, fallback?: string, defaultSlug = ''): string => {
...
export const mediaReferenceExists = (value: string | undefined, hasMediaById: (id: string) => boolean): boolean => {
...
export const isValidMediaFieldValue = (
  ...
  options?: { allowInlineText?: boolean; hasMediaById?: (id: string) => boolean },
): boolean => {
...
export const normalizeStringArray = (value: unknown): string[] =>
  Array.isArray(value) ? value.map((entry) => requiredTrimmed(entry)).filter(Boolean) : [];
